...
const appState = {
  route: window.MANAGER_APP ? "/admin" : window.location.pathname,
  entryStore: null,
  customerStores: [],
  adminStores: [],
  customerSearch: "",
...
async function apiFetch(path, options = {}) {
  ...
  if (!response.ok) {
    ...
    if (window.MANAGER_APP && response.status === 401 && appState.adminAuthenticated) {
      window.ManagerWeb.sessionExpired();
    }
...
function renderServiceChoice(mode, selectedStore) {
  ...
  const serviceButtons = SERVICE_ORDER.map((serviceType) => {
    ...
    const label = window.MANAGER_APP ? (serviceType === "simple_service" ? "간단서비스" : "구매상담") : (mode === "admin" ? meta.admin_label : meta.customer_label);
  ...
  if (!window.MANAGER_APP) serviceButtons.push(`<button class="btn ${compactChoice ? "admin-choice-integrated" : "btn-green"}" data-action="${action}" data-service="${INTEGRATED_SERVICE}">통합보기</button>`);
  ...
  return `
    <div class="modal-backdrop" data-action="closeAdminSettings">
      <section class="modal viewer-settings-modal admin-settings-modal" role="dialog" aria-modal="true" aria-label="관리자 설정" data-action="stopModalClose">
        ...
        <div class="viewer-settings-actions">
          ${pushButton}
          ${selectedStore && !window.MANAGER_APP ? `<a class="btn btn-ghost" href="${customerUrl}" target="_blank" rel="noopener">모니터링 화면</a>` : ""}
          ...
          ${window.MANAGER_APP ? "" : '<button class="btn btn-ghost" type="button" data-link="/">처음으로</button>'}
        </div>
      </section>
    </div>
  `;
...
function renderCustomerDisplayServiceCard(serviceType, scope = "customer") {
  ...
  const visibleNumber = serviceState?.current_number ?? null;
  const nextCallNumber = serviceState?.next_waiting_number ?? null;
  ...
  return `
    <article class="service-card viewer-service-card ${theme}">
      ...
      <div class="current-box">
        ${visibleNumber !== null ? `<div class="current-number ${theme}">${visibleNumber}</div>` : `<div class="current-number wait">대기중</div>`}
      </div>
      <div class="state-mini ${showWaitingStatus ? "" : "single-state"}">
        <div class="state-pill"><span>대기 인원</span><strong>${waitingCount}명</strong></div>
        <div class="state-pill"><span>다음 호출번호</span><strong>${nextCallNumber !== null ? `${nextCallNumber}번` : "없음"}</strong></div>
        ${showWaitingStatus ? `<div class="state-pill"><span>대기 상태</span>${renderWaitingStatus(waitingCount)}</div>` : ""}
      </div>
      <div class="queue-title">대기현황</div>
...
function renderAdmin() {
  if (window.MANAGER_APP && (!appState.adminAuthenticated || !appState.selectedAdminStore)) {
    window.ManagerWeb.renderEntry();
    return;
  }
...
function renderAdminServiceCard(serviceType) {
  ...
  const theme = meta.theme;
  const current = serviceState?.current_number ?? null;
  const waitingCount = serviceState?.waiting_count ?? 0;
  const nextCallNumber = serviceState?.next_waiting_number ?? null;
  return `
    <article class="service-card ${theme}">
      ...
      <div class="current-box">
        ${current !== null ? `<div class="current-number ${theme}">${current}</div>` : `<div class="current-number wait">대기중</div>`}
      </div>
...
function isPushSupported() {
  if (window.MANAGER_APP) return false;
...
function primeAdminAudio() {
  if (window.MANAGER_APP) return null;
...
$app.addEventListener("click", (event) => {
  ...
  if (action === "logoutAdmin") {
    if (window.MANAGER_APP) {
      safeRun(() => window.ManagerWeb.logout());
      return;
    }
    appState.adminSettingsOpen = false;
    safeRun(async () => {
  ...
  if (action === "selectAdminService") {
    if (window.MANAGER_APP) { window.ManagerWeb.openService(serviceType); return; }
    appState.adminSettingsOpen = false;
  ...
  if (action === "changeAdminService") {
    if (window.MANAGER_APP) { window.ManagerWeb.chooseService(); return; }
    appState.adminSettingsOpen = false;
    appState.selectedAdminService = null;
    renderAdmin();
    if (appState.selectedAdminStore) {
  ...
  if (action === "changeAdminStore") {
    if (window.MANAGER_APP) { safeRun(() => window.ManagerWeb.logout()); return; }
    appState.adminSettingsOpen = false;
    clearPolling();
    appState.selectedAdminStore = null;
    appState.selectedAdminService = null;
