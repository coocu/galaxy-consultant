...
from .mobile_push import FirebaseSender, ensure_mobile_schema, install_mobile_push, mobile_push_loop, register_mobile_routes
from .push import (
...
def create_app(test_config: dict | None = None) -> FastAPI:
    ...
    async def lifespan(_app: FastAPI):
        ...
        mobile_task = None
        if _app.state.mobile_settings.enabled:
            sender = _app.state.mobile_sender
            if isinstance(sender, FirebaseSender):
                try:
                    await asyncio.to_thread(sender.prepare)
                except Exception:
                    # 번호표 서비스는 유지하고 관리자 상태 API에서 설정 오류를 확인한다.
                    import logging
                    logging.getLogger("uvicorn.error").warning(
                        "Manager FCM configuration needs attention: %s", sender.error)
            if not settings.testing:
                mobile_task = asyncio.create_task(mobile_push_loop(_app))
        ...
        try:
            ...
            if mobile_task is not None:
                ...
                with suppress(asyncio.CancelledError):
                    await mobile_task
            if isinstance(_app.state.mobile_sender, FirebaseSender):
                _app.state.mobile_sender.close()
    ...
    install_mobile_push(app, test_config)
    register_mobile_routes(app, require_admin)
    ...
    def manager_index(request: Request) -> HTMLResponse:
        response = TEMPLATES.TemplateResponse(
            request, "index.html", {"request": request, "service_meta": SERVICE_META, "manager_app": True},
        )
