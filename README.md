...
- 메인 화면: 고객 / 관리자 버튼 2개
- 고객 화면: 매장 검색, 매장 선택, 간단서비스/구매문의/통합보기 업무 선택, 선택한 업무의 호출 표시
- 관리자 화면: CodeNote 인증키 로그인, 매장 검색/선택, 갤럭시 컨설턴트/구매상담/통합보기 업무 선택, 선택한 업무만 호출
...
- 통합보기: 간단서비스와 구매문의를 좌우 2분할로 함께 표시하며 모바일에서는 세로로 자동 전환
- 고객 호출 화면 설정: 음성 켜짐, 업무 변경, 매장 변경은 오른쪽 상단 톱니바퀴 팝업에서 사용
- 관리자 화면 설정: 발급 알림, 고객화면, 업무 변경, 매장 변경, 매장관리, 로그아웃, 처음으로는 오른쪽 상단 톱니바퀴 팝업에서 사용
- 대기 목록: 고객 호출 화면에서만 표시
- 매장 관리: 매장 생성, 매장 이름/점코드 수정, 실제 삭제, 매장명/점코드 검색
...
/          메인
/customer  고객 호출 화면
/customer?store_id=매장ID&service_type=integrated  고객 통합보기 화면
/admin     관리자 호출/매장관리
...
관리자 로그인 후 오른쪽 상단 톱니바퀴를 누르고 `매장관리` 버튼을 누릅니다.
...
백업 ZIP에는 매장 카테고리만 들어갑니다. 번호표, 현재 호출번호, 호출기록, 업무별 카운터는 백업하지 않습니다. 서버 매장관리에서 삭제하면 해당 매장 카테고리와 해당 매장의 번호표/호출/푸시 구독 데이터가 같이 삭제됩니다.
